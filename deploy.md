# GitHub Actions 自动部署指南

> ✅ **已实现功能**: 每次 `git push` 自动部署到服务器，无需手动操作

## 🎯 快速开始

| 步骤 | 操作 | 执行次数 |
|------|------|----------|
| **1** | 本地生成 SSH 密钥 | 一次 |
| **2** | 服务器运行一键脚本 | 一次 |
| **3** | GitHub 添加 Secrets | 一次 |
| **4** | `git push` 自动部署 | 每次提交 |

---

## 📋 详细流程

```
┌─────────────────┐     ┌─────────────────┐     ┌─────────────────────────┐
│  1. 生成密钥    │────→│  2. 服务器初始化 │────→│  3. 自动部署（已完成）   │
│   (本地执行)    │     │  (服务器执行一次) │     │  每次 git push 自动触发  │
└─────────────────┘     └─────────────────┘     └─────────────────────────┘
```

## 📋 前置要求

### 1. 本地生成 SSH 密钥对

在本地电脑生成密钥对（不要设置密码）：

```bash
ssh-keygen -t ed25519 -C "github-actions" -f ~/.ssh/github_actions
# 一路回车，不设置密码
```

会生成两个文件：
- `~/.ssh/github_actions`（私钥）→ 复制内容到 GitHub Secrets `SSH_PRIVATE_KEY`
- `~/.ssh/github_actions.pub`（公钥）→ 添加到服务器

### 2. 服务器端一键部署

**方式一：使用一键部署脚本（推荐）**

```bash
# 1. 上传公钥到服务器
ssh-copy-id -i ~/.ssh/github_actions.pub root@你的服务器IP

# 2. 登录服务器
ssh root@你的服务器IP

# 3. 下载部署脚本（首次部署后可直接运行项目中的脚本）
curl -fsSL -o deploy.sh https://raw.githubusercontent.com/你的用户名/你的仓库/main/deploy.sh
bash deploy.sh
```

> 💡 **提示**: 脚本运行后会提示输入 Git 仓库地址，然后自动完成所有配置。数据库和 `.env` 配置会有交互式提示。

**方式二：手动部署**

```bash
# 1. 上传公钥到服务器
ssh-copy-id -i ~/.ssh/github_actions.pub root@你的服务器IP

# 2. 登录服务器
ssh root@你的服务器IP

# 3. 安装 Node.js 18+
curl -fsSL https://deb.nodesource.com/setup_18.x | sudo -E bash -
apt-get install -y nodejs

# 4. 安装 PM2
npm install -g pm2

# 5. 克隆项目
git clone https://github.com/你的用户名/你的仓库.git /var/www/back-master
cd /var/www/back-master

# 6. 安装依赖
npm ci --production

# 7. 配置环境变量
cp .env.example .env
nano .env

# 8. 启动
pm2 start ecosystem.config.js --env production
pm2 save
pm2 startup
```

### 3. 配置 GitHub Secrets

在 GitHub 仓库页面，进入 **Settings → Secrets and variables → Actions**，添加以下 Secrets：

| Secret 名称 | 说明 | 示例 |
|------------|------|------|
| `SERVER_HOST` | 服务器 IP 或域名 | `192.168.1.100` |
| `SERVER_USER` | SSH 登录用户名 | `root` 或 `ubuntu` |
| `SSH_PRIVATE_KEY` | SSH 私钥内容 | 见下方生成方法 |
| `SERVER_PORT` | SSH 端口（可选） | `22` |
| `DEPLOY_PATH` | 项目部署路径 | `/var/www/back-master` |



## 🚀 自动部署（已完成配置）

### 触发条件

配置完成后，以下操作会自动触发部署：
| 操作 | 触发结果 |
|------|----------|
| `git push origin main` | ✅ 自动部署 |
| PR 合并到 `main`/`master` | ✅ 自动部署 |

### 本地开发流程

```bash
# 1. 修改代码
git add .
git commit -m "feat: 新增功能"

# 2. 推送代码 → 自动触发部署
git push origin main

# 3. 等待约 30 秒，部署完成
# 可在 GitHub 仓库 → Actions 标签页查看部署日志
```

### GitHub Actions 自动执行的步骤

```
┌─────────────────────────────────────────────────────────┐
│  GitHub Actions 自动部署流程                              │
├─────────────────────────────────────────────────────────┤
│  1. 检出代码                                             │
│  2. 安装 Node.js 依赖（用于测试，可选）                   │
│  3. SSH 连接到服务器                                      │
│  4. 执行 git pull 拉取最新代码                            │
│  5. 执行 npm ci --production 安装生产依赖                 │
│  6. 执行 pm2 reload 平滑重启应用（零停机）                 │
│  7. 保存 PM2 进程列表                                     │
└─────────────────────────────────────────────────────────┘
```

## ✅ 验证自动部署

### 查看部署状态

1. **GitHub 仓库** → **Actions** 标签页
2. 点击最新的工作流运行记录
3. 查看实时日志

### 查看应用状态（服务器）

```bash
# 查看应用运行状态
pm2 status

# 查看应用日志
pm2 logs back-master

# 查看实时日志（最新 100 行）
pm2 logs back-master --lines 100
```

## 🔧 手动部署

如果需要手动触发部署，可以在 GitHub 页面：
1. 进入 **Actions** 标签
2. 选择 **Deploy to Server** 工作流
3. 点击 **Run workflow**

## 📝 注意事项

### 1. 数据库配置

确保服务器上的 `.env` 文件已正确配置数据库连接信息。

### 2. 防火墙设置

确保服务器防火墙允许：
- SSH 连接（默认 22 端口）
- 应用端口（默认 3007）

```bash
# 示例：开放 3007 端口
sudo ufw allow 3007
```

### 3. Nginx 反向代理（推荐）

如果需要使用域名和 HTTPS，建议配置 Nginx：

```nginx
server {
    listen 80;
    server_name your-domain.com;
    
    location / {
        proxy_pass http://127.0.0.1:3007;
        proxy_http_version 1.1;
        proxy_set_header Upgrade $http_upgrade;
        proxy_set_header Connection 'upgrade';
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
        proxy_cache_bypass $http_upgrade;
    }
}
```

### 4. 日志查看

```bash
# 查看 PM2 日志
pm2 logs back-master

# 查看应用日志
tail -f logs/out.log
tail -f logs/error.log
```

## 🔍 故障排查

### 部署失败

1. 检查 GitHub Actions 日志
2. 确认 Secrets 配置正确
3. 检查服务器 SSH 连接是否正常

### 应用启动失败

1. 检查 `.env` 配置
2. 检查数据库连接
3. 查看 PM2 日志：`pm2 logs`

### 端口占用

```bash
# 查看端口占用
sudo lsof -i :3007

# 杀死进程
sudo kill -9 <PID>
```

## 📚 相关文档

- [PM2 文档](https://pm2.keymetrics.io/docs/usage/quick-start/)
- [GitHub Actions 文档](https://docs.github.com/cn/actions)
